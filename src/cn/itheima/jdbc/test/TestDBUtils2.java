...
import cn.itheima.domain.lable; 
...
public class TestDBUtils2 {
	...
	public void testQueryAll() {
		try {
			...
			// 4.对结果集集合进行遍历
			for (lable l : lables) {
				System.out.println(l.getLtitle()+ " : " + l.getlContent());
			}
	...
	public void testQueryUserById() {
		try {
			...
			Object[] params = {i};
			// 4.执行查询操作
			User user = qr.query(sql, new BeanHandler<User>(User.class), i);
	...
	public void testQueryCount() {
		try {
			...
			QueryRunner qr = new QueryRunner(C3P0Utils.getDataSource());
			// 2.编写sql语句
			String sql = "select count(*) from user where u_tel = ? and u_pwd = ? ";
			String u_tel = "15733692827";
			String u_pwd = "123456";
			Object[] params = {u_tel,u_pwd};
			// 4.执行查询操作
